...
*** 个人文件
个人文件用来保存用户曾经输入过的中文词条以及这些词条输入的先后顺序（也就是词频信息）。
Chinese-pyim 搜索中文词条时，个人文件里的词条优先显示。我们使用变量 `pyim-personal-file'
来保存个人文件的路径，在使用 pyim-load-file 加载词库文件的时候，如果这个文件不存在，
Chinese-pyim 会使用函数：`pyim-create-template-dict' 自动创建这个文件。

个人文件使用上述词库文件的格式来保存上述信息，将其独立出来的原因是：
1. 随着 `Chinese-pyim' 使用时间的延长，个人文件会保存越来越多的用户常用的词条，属于
   用户隐私，提醒用户不要随意将这个文件泄露他人。
2. 个人文件的内容在 Chinese-pyim 使用过程中频繁的变动。
3. 随着个人文件的积累，Chinese-pyim 会越来越顺手，所以个人文件需要用户经常备份。
...
*** 标点符号列表
因为常用的标点符号数量不多，所以 Chinese-pyim 没有使用文件词库来处理
半角标点符号和全角标点符号，而是使用一个变量 `pyim-punctuation-dict' 来处理
标点符号样式的转换，这个变量是一个alist。在调用函数 `pyim-default-translate'
的时候进行标点符号格式的转换。
...
的快捷键都 *名花有主* 了，甚至找一个的 “Ctrl＋单字符”的快捷键都不太容易，特殊功能触发
...
2. 快速将光标前的词条添加到词库：当光标前的字符是中文字符时，按 "num" + "v" 可以将光标前 num 个中文汉字
   组成的词条添加到个人文件中，比如：当光标在A处时，按"4v"可以将“的红烧肉”这个词条加入个人文件，默认
   num不超过9。
   #+BEGIN_EXAMPLE
   我爱吃美味的红烧肉-A-
   #+END_EXAMPLE
...
假如：用户输入了一个错误的拼音“ying-gai”，用户可以通过快捷键运行这个函数，将“ing” 替换 “in”，
得到 “yin-gai”对应的词语。

这种处理方式能力有限，一次不能处理太多的模糊音，用户需要根据自己的需要，自定义模糊音处理函数。

自定义模糊音处理函数可以参考：`pyim-pinyin-fuzzy-adjust-1 的定义方式。
#+BEGIN_SRC emacs-lisp :noweb yes :tangle chinese-pyim.el
