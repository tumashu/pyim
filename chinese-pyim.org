...
许多时候，我们必须根据环境的具体情况来自动开启或者关闭中文输入，比如：当 org-mode 用户
激活 speed commands 后，光标在A位置时：必须自动关闭中文输入功能，

#+BEGIN_EXAMPLE
,-A-** HEAD
#+END_EXAMPLE

而光标在B位置时，必须自动开启中文输入功能。
#+BEGIN_EXAMPLE
,** -B-HEAD
#+END_EXAMPLE
...
Chinese-pyim 在插入字符之前，会运行 pyim-english-input-switch-function
对应的函数，当返回值为 t 时，Chinese-pyim 就会开启英文输入模式，当这个
函数返回值为 nil 时，Chinese-pyim 就会开启中文输入模式。

值得注意的是，pyim-toggle-input-ascii 这个命令也可以控制输入法是否开启英文
输入模式，但两者使用的情况不同，用户一般将 pyim-toggle-input-ascii 绑定到
特定的快捷键上，手动切换中英文输入模式。这个命令通过变量 pyim-input-ascii
来实现切换。

Chinese-pyim 使用 pyim-input-chinese-p 函数来决定是否开启英文输入模式，
上述两种方式共同影响 pyim-input-chinese-p 的结果，最终直观的行为是：
