...
#+NAME: variables-load-hook-and-active-hook
#+BEGIN_SRC emacs-lisp :noweb yes
...
#+NAME: functions-pyim-start-and-restart
#+BEGIN_SRC emacs-lisp :noweb yes
...
#+NAME: functions-save-personal-file
#+BEGIN_SRC emacs-lisp :noweb yes
...
#+NAME: functions-check-or-delete-dicts-buffer
#+BEGIN_SRC emacs-lisp :noweb yes
...
#+NAME: functions-search-from-dicts
#+BEGIN_SRC emacs-lisp :noweb yes
...
#+NAME: functions-add-a-word-to-personal-buffer
#+BEGIN_SRC emacs-lisp :noweb yes
...
#+NAME: functions-delete-a-word-from-personal-buffer
#+BEGIN_SRC emacs-lisp :noweb yes
...
#+NAME: functions-adjuct-a-word-position-in-personal-buffer
#+BEGIN_SRC emacs-lisp :noweb yes
...
#+NAME: pyim-punctuation-escape-list
...
#+NAME: pyim-toggle-full-width-punctuation
#+BEGIN_SRC emacs-lisp :noweb yes
;;; 切换中英文标点符号
...
#+NAME: pyim-punctuation-translate-p
#+BEGIN_SRC emacs-lisp :noweb yes
...
#+NAME: pyim-punctuation-translate-at-point
#+BEGIN_SRC emacs-lisp :noweb yes
;; 切换光标处标点的样式（全角 or 半角）
...
#+BEGIN_SRC emacs-lisp :results value scalar replace
...
#+NAME: pyim-return-proper-punctuation
#+BEGIN_SRC emacs-lisp :noweb yes
;; 处理标点符号
...
#+NAME: pyim-pair-punctuation-status
#+BEGIN_SRC emacs-lisp :noweb yes
...
构建为一个字符串，然后使用 `message' 命令在 minibuffer 中显示这个字符串。
...
`pyim-current-pos' 的取值以及 `pyim-page-length' 的设定值，两者共同决定了 Chinese-pyim 需要
...
5. 函数 `pyim-format-page' 会从 `pyim-current-choices' 中提取一个sublist:
   ...
   然后使用这个 sublist 来构建 `pyim-guidance-str' :
   #+BEGIN_EXAMPLE
   "1. 薿 2.旎 3.睨 4.铌 5.昵 6.匿 7.倪 8.霓 9.暱"
   #+END_EXAMPLE

`pyim-next-page' 这个命令用来翻页，其基本原理就是改变 `pyim-current-pos'的取值，假设一次只翻一页，
那么这个函数所做的工作就是：
...
2. 然后将 `pyim-current-pos' 的值设定为 `pyim-page-start' 的返回值，确保 `pyim-current-pos'
   的取值为下一页第一个词条的位置。
3. 最后调用 `pyim-format-page' 来更新 `pyim-guidance-str' 。

#+NAME: functions-format-page
#+BEGIN_SRC emacs-lisp :noweb yes
;;;  page format
...
#+NAME: pyim-translate-trigger-char
#+BEGIN_SRC emacs-lisp :noweb yes
...
#+NAME: pyim-fuzzy-pinyin-adjust-function
#+BEGIN_SRC emacs-lisp :noweb yes
...
#+NAME: pyim-fuzzy-pinyin-adjust-1
#+BEGIN_SRC emacs-lisp :noweb yes
...
#+NAME: variables-shenmu-and-yumu
#+BEGIN_SRC emacs-lisp :noweb yes
(defvar pyim-shen-mu
...
#+NAME: functions-pinyin-string-manupulate
#+BEGIN_SRC emacs-lisp :noweb yes
;; 将汉字的拼音分成声母和其它
...
#+BEGIN_SRC emacs-lisp :eval yes :results value scalar replace
...
<<pyim-fuzzy-pinyin-adjust-function>>
...
#+BEGIN_SRC emacs-lisp :noweb yes :tangle chinese-pyim.el
;;;  variable declare
<<variables-load-dicts>>

<<variables-shenmu-and-yumu>>
...
<<variables-load-hook-and-active-hook>>
...
** 函数和命令
#+BEGIN_SRC emacs-lisp :noweb yes :tangle chinese-pyim.el
<<functions-load-dicts>>

<<functions-save-personal-file>>

<<functions-search-from-dicts>>

<<function-get-words>>

<<functions-add-or-delete-words>>

<<functions-add-a-word-to-personal-buffer>>

<<functions-delete-a-word-from-personal-buffer>>

<<functions-adjuct-a-word-position-in-personal-buffer>>

;;;  interface
<<functions-check-or-delete-dicts-buffer>>

;;;  pinyin manupulate
<<functions-pinyin-string-manupulate>>

;;; char code
<<functions-get-char-code>>

;;; punctuation manupulate
<<functions-translate>>
...
<<functions-pyim-start-and-restart>>
