...
(defcustom pyim-dicts nil
  "一个列表，用于保存 `Chinese-pyim' 的词库信息，用户可以使用 `pyim-add-dict'
命令来设定这个变量。
...
`pyim-dicts' 每一个 element 都代表一条词库的信息。词库信息使用一个 plist 来
表示，比如：
            ...
            (cond
             ...
             ((stringp dict)
              ...
              (if (and (file-exists-p file)
                ...
                (message "忽略导入重复的词库文件：%s。" file)))))
        (warn "Chinese-pyim 没有可用词库，输入中文词语的功能极度受限，请按`C-h v pyim-dicts'查看详细信息。")))
    buflist))
