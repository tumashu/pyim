...
1.  [pyim 使用说明](#org1c85028)
    ...
    7.  [配置](#org247f692)
        1.  [配置实例](#orgba52af8)
        2.  [添加词库文件](#org95a9b6a)
        3.  [激活 pyim](#orgbd598ec)
    8.  [使用](#org62013b6)
        1.  [常用快捷键](#org1732252)
        2.  [使用双拼模式](#org23ff42f)
        3.  [通过 pyim 来支持 rime 所有输入法](#org185dd36)
        4.  [使用五笔输入](#orga8ead78)
        5.  [使用仓颉输入法](#org43fe00d)
        6.  [让选词框跟随光标](#org2f72dc5)
        7.  [调整 tooltip 选词框的显示样式](#orgc85cb8e)
        8.  [设置模糊音](#org3ec4c17)
        9.  [使用魔术转换器](#org169bf02)
        10. [切换全角标点与半角标点](#org5bd6ea3)
        11. [手动加词和删词](#orga3e31a7)
        12. [pyim 高级功能](#org06c6bc0)
    9.  [捐赠](#orgc06f7c2)
    10. [Tips](#org34d74c3)
        1.  [关闭输入联想词功能 (默认开启)](#org1b89a09)
        2.  [如何将个人词条相关信息导入和导出？](#org5e2c14f)
        3.  [pyim 出现错误时，如何开启 debug 模式](#org8f85f28)
        4.  [如何查看 pyim 文档。](#org00d0db6)
        5.  [将光标处的拼音或者五笔字符串转换为中文 (与 vimim 的 “点石成金” 功能类似)](#org6a419a9)
        6.  [如何使用其它字符翻页](#orgf5c49c7)
        7.  [如何用 ";" 来选择第二个候选词](#org67102f7)
        8.  [如何添加自定义拼音词库](#org5d5681a)
        9.  [如何手动安装和管理词库](#org5daf054)
        10. [Emacs 启动时加载 pyim 词库](#orgd9cd2d1)
        11. [将汉字字符串转换为拼音字符串](#org47d435e)
        12. [中文分词](#orgfbeb7de)
        13. [获取光标处的中文词条](#org2659481)
        14. [让 \`forward-word' 和 \`back-backward’ 在中文环境下正常工作](#org979ac31)
        15. [为 isearch 相关命令添加拼音搜索支持](#org71ec39e)
...
如果 pyim-basedict 不能满足需求，用户可以使用其他方式为 pyim 添加拼音词库，具体方式请参考 [1.10.8](#org5d5681a) 小结。
...
pyim 使用 emacs 动态模块：[liberime](https://github.com/merrickluo/liberime)
来支持 rime, 设置方式：
...
2.  创建文件： "~/.emacs.d/pyim/rime/default.custom.yaml", 内容为：
    ...
    \`liberime-get-context' 函数在默认情况下一次只能获取5个候选词，如果想获取更多的候选词，就需要给 liberime 发送翻页命令字符，模拟翻页操作，会让 pyim 代码的维护难度增加许多，所以 pyim 使用了一种简单粗暴的方式来处理这个问题： 将 rime 的 page\_size 设置为 50, 这样，pyim 在不处理 rime 分页的情况下, 一次就可以获取 50 个候选词，然后用自己方式来分页。用户可以按 TAB 键切换到辅助输入法来输入 50
    以后的词条。
    
    更改配置这种方式有点 hack, 如果 liberime 能设置 menu/page\_size
    就好了。
...
5.  如果通过 rime 使用微软双拼，可以用以下设置：
    ...
    默认是用繁体中文，想要改成简体中文的话，可以参考 [rime wiki](https://github.com/rime/home/wiki/CustomizationGuide#一例定製簡化字輸出)，或者[这篇博客](http://wenshanren.org/?p=1070#orgc7dbd8e)
