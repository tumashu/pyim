    ...
    (if (pyim-use-emacs-thread-p)
        (make-thread
         `(lambda ()
            (maphash
             #'(lambda (key value)
                 (dolist (x (pyim-dhashcache-return-shortcode key))
                   (puthash x
                            (mapcar
                             #'(lambda (word)
